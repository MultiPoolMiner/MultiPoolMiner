...
$HashRefinery_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Where-Object {$DisabledAlgorithms -inotcontains (Get-Algorithm $HashRefinery_Request.$_.name) -and $Hashrefinery_Request.$_.hashrate -gt 0} | ForEach-Object {
