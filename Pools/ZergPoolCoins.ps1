...
$ZergPoolCoins_MiningCurrencies | Where-Object {$ExcludeCoin -inotcontains $ZergPoolCoins_Request.$_.name -and $ExcludeAlgorithm -inotcontains (Get-Algorithm $ZergPoolCoins_Request.$_.algo) -and ($Coin.count -eq 0 -or $Coin -icontains $ZergPoolCoins_Request.$_.name) -and $ZergPoolCoins_Request.$_.hashrate -gt 0} | ForEach-Object {
    ...
    $Stat = Set-Stat -Name "$($Name)_$($ZergPoolCoins_Currency)_Profit" -Value ([Double]$ZergPoolCoins_Request.$ZergPoolCoins_Currency.estimate / $Divisor) -Duration $StatSpan -ChangeDetection $true
