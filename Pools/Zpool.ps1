...
$URL = "http://www.zpool.ca/api/status"
...
# Cannot do SSL
if ($SSL) {Write-Warning "SSL option requested, but pool API ($Name) does not support SSL. Ignoring pool.";return}

$Zpool_Request = [PSCustomObject]@{}
try {
    $Zpool_Request = Invoke-RestMethod $URL -UseBasicParsing -TimeoutSec 15 -ErrorAction Stop
}
catch {
...
$Zpool_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select -ExpandProperty Name | ForEach-Object {
