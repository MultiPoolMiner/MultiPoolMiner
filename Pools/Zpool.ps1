...
#    $MiningCurrency = $(if ($APIStatusResponse.$_.coins -eq 1) { ($APICurrenciesResponse | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Where-Object { $APICurrenciesResponse.$_.name -eq $CoinName -and $APICurrenciesResponse.$_.algo -eq $Algorithm} ) -split "-" | Select-Object -First 1 } )
