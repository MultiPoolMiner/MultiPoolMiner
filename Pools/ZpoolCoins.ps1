...
while (-not ($APIStatusResponse -and $APICurrenciesResponse) -and $RetryCount -gt 0) {
    try {
        ...
        if (-not $APICurrenciesResponse) {Start-Sleep 10 <#Temp fix: workaround for Zpool(Coins) anti-DDoS protect (pool allows only 6 API calls per minute)#>; $APICurrenciesResponse = Invoke-RestMethod $PoolAPICurrenciesUri -UseBasicParsing -TimeoutSec 3 -ErrorAction Stop}
    }
    catch {
