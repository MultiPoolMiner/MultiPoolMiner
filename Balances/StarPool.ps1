...
while (-not ($APIRequest) -and $RetryCount -gt 0) {
    try {
        if (-not $APIRequest) {$APIRequest = Invoke-RestMethod "https://www.starpool.biz/api/currencies" -UseBasicParsing -Headers @{"Cache-Control" = "no-cache"} -TimeoutSec 3 -ErrorAction Stop}
    }
    catch {
...
$Payout_Currencies = @("BTC", "CANN", "LTC", "DASH")
...
$Payout_Currencies | Foreach-Object {
    try {
        $APIRequest = Invoke-RestMethod "https://www.starpool.biz/api/wallet?address=$($PoolConfig.Wallets.$_)" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
        if (($APIRequest | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Measure-Object Name).Count -le 1) {
