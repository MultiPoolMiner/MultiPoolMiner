...
$HashSHA256 = "E574C8B7FFFBD271AC45B568CEB48D134DE896E24E81AE8E6A56FBF6F69CF1DA"
$Uri = "https://github.com/Lolliedieb/lolMiner-preview/releases/download/alpha2/lolMiner_v06_alpha2_Win64.zip"
...
$Commands = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{Algorithm = "Equihash1445"; MinMemGB = 1.85; Fee = 2; Params = ""}
    ...
    $Commands | Where-Object {$Pools.(Get-Algorithm $_.Algorithm).Protocol -eq "stratum+tcp" <#temp fix#>} | ForEach-Object {
        ...
        if ($Miner_Device = @($Device | Where-Object {$_.OpenCL.GlobalMemsize -ge ($MinMem)})) {
            ...
            if ($Coin) {
                ...
                $ConfigFileName = "$((@($Pools.$Algorithm_Norm.Name) + @($Algorithm_Norm) + @($Coin) | Select-Object) -join '-').json"
