    ...
    $Commands | ForEach-Object { $Algorithm_Norm = @(@(Get-Algorithm ($_.Algorithm -split '-' | Select-Object -First 1) | Select-Object) + @($_.Algorithm -split '-' | Select-Object -Skip 1) | Select-Object -Unique) -join '-'; $_ } | Where-Object { $Pools.$Algorithm_Norm.Host } | ForEach-Object { 
        ...
        if ($Miner_Device = @($Device | Where-Object { ([math]::Round((10 * $_.OpenCL.GlobalMemSize / 1GB), 0) / 10) -ge $MinMemGB })) { 
            ...
            if ($Miner_Config.Parameters.$Algorithm_Norm) { 
                ...
                if ($Miner_Config.Parameters.$SecondaryAlgorithm_Norm -and $SecondaryAlgorithm_Norm -and $_.SecondaryAlgoIntensity -gt 0) { 
                    $Parameters += Get-CommandPerDevice $Miner_Config.Parameters.$($SecondaryAlgorithm_Norm) $Miner_Device.Type_Vendor_Index
                }
            }
            ...
            if ($null -ne $_.SecondaryAlgoIntensity) { 
                ...
                if ($Miner_Device | Where-Object { $_.OpenCL.GlobalMemsize -gt 3GB }) { 
                    $Miner_Fees = [PSCustomObject]@{ $Algorithm_Norm = 1 / 100; $SecondaryAlgorithm_Norm = 0 / 100 }
                }
                else { 
                    $Miner_Fees = [PSCustomObject]@{ $Algorithm_Norm = 0 / 100; $SecondaryAlgorithm_Norm = 0 / 100 }
                }
            }
            else { 
