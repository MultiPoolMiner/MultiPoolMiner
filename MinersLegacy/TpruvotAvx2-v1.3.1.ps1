...
$Devices | Select-Object Model -Unique | ForEach-Object {
    ...
    $Commands | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name | ForEach-Object {$Algorithm_Norm = Get-Algorithm $_; $_} | Where-Object {$Pools.$Algorithm_Morm.Protocol -eq "stratum+tcp" <#temp fix#>} | ForEach-Object {
        ...
        [PSCustomObject]@{
            ...
            Arguments  = ("-a $_ -o $($Pools.$Algorithm_Norm.Protocol)://$($Pools.$Algorithm_Norm.Host):$($Pools.$Algorithm_Norm.Port) -u $($Pools.$Algorithm_Norm.User) -p $($Pools.$Algorithm_Norm.Pass) -b $Miner_Port$Param$CommonParameters" -replace "\s+", " ").trim()
