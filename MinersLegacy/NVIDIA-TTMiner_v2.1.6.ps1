    ...
    $Commands | Where-Object {$Pools.(Get-Algorithm ($_.Algorithm -split '-' | Select-Object -Index 0)).Protocol -eq "stratum+tcp" <#temp fix#>} | ForEach-Object {
        ...
        if ($Miner_Device = @($Device | Where-Object {([math]::Round((10 * $_.OpenCL.GlobalMemSize / 1GB), 0) / 10) -ge $MinMemGB})) {
            ...
            [PSCustomObject]@{
                ...
                Arguments          = ("--api-bind 127.0.0.1:$($Miner_Port) -A $Algorithm -P $([System.Web.HttpUtility]::UrlEncode($Pools.$Algorithm_Norm.User)):$([System.Web.HttpUtility]::UrlEncode($Pools.$Algorithm_Norm.Pass))@$($Pools.$Algorithm_Norm.Host):$($Pools.$Algorithm_Norm.Port)$($Commands.$_)$CommonCommands -d $(($Miner_Device | ForEach-Object {'{0:x}' -f ($_.Type_Vendor_Index)}) -join ' ')" -replace "\s+", " ").trim()
