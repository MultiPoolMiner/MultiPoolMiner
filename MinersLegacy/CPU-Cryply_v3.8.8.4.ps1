...
$Devices | Select-Object Model -Unique | ForEach-Object {
    ...
    $Paths = @()
    if ($Miner_Device.CpuFeatures -match "avx")               {$Paths += ".\Bin\CPU-Cryply_v3.8.8.4\cpuminer-Avx.exe"}
    if ($Miner_Device.CpuFeatures -match "(avx2|[^sha]){2}")  {$Paths += ".\Bin\CPU-Cryply_v3.8.8.4\cpuminer-Avx2.exe"}
    if ($Miner_Device.CpuFeatures -match "(avx2|sha){2}")     {$Paths += ".\Bin\CPU-Cryply_v3.8.8.4\cpuminer-Avx2-Sha.exe"}
    if ($Miner_Device.CpuFeatures -match "sse2")              {$Paths += ".\Bin\CPU-Cryply_v3.8.8.4\cpuminer-Sse2.exe"}
    if ($Miner_Device.CpuFeatures -match "(aes|sse42){2}")    {$Paths += ".\Bin\CPU-Cryply_v3.8.8.4\cpuminer-Aes-Sse42.exe"}
    if (-not $Paths) {$Paths = @(".\Bin\CPU-Cryply_v3.8.8.4\cpuminer.exe")}
