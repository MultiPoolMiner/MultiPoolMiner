    ...
    $Commands | ForEach-Object {
        ...
        if ($Arguments_Platform -and $Miner_Device) {
            if ($_.SecondaryAlgorithm) {
                ...
                $Miner_Name = (@($Name) + @("$($MainAlgorithm_Norm -replace '^ethash', '')$SecondaryAlgorithm_Norm") + @(if ($_.SecondaryIntensity -ge 0) {$_.SecondaryIntensity}) + @($Miner_Device.Model_Norm | Sort-Object -Unique) | Select-Object) -join '-'
