    ...
    $Commands | ForEach-Object {
        ...
        if ($Arguments_Platform) {
            if ($_.SecondaryAlgorithm) {
                ...
                $Miner_Name = (@($Name) + @("$SecondaryAlgorithm_Norm") + @(if ($_.SecondaryIntensity -ge 0) {$_.SecondaryIntensity}) + @($Miner_Device.Model_Norm | Sort-Object) | Select-Object -Unique) -join '-'
                ...
                $Miner_Name = ((@($Name) + @($Miner_Device.Model_Norm | Sort-Object) | Select-Object -Unique) -join '-') -replace "[-]{2,}", "-"
            ...
            [PSCustomObject]@{
                ...
                Arguments  = ("-mport -$Miner_Port -epool $($Pools.$MainAlgorithm_Norm.Host):$($Pools.$MainAlgorithm.Port) -ewal $($Pools.$MainAlgorithm_Norm.User) -epsw $($Pools.$MainAlgorithm_Norm.Pass) -allpools 1 -allcoins exp -esm 3$Arguments_Secondary$($_.Params)$Arguments_Platform -di $(($Miner_Device | ForEach-Object {'{0:x}' -f $_.Vendor_Index}) -join '')" -replace "\s+", " ").trim()
