...
) -replace "^{" -replace "}$" | Set-Content "$(Split-Path $Path)\$($Pools.CryptoNight.Name)_CryptoNight.txt" -Force -ErrorAction SilentlyContinue

[PSCustomObject]@{
    Type      = "AMD"
    Path      = $Path
    Arguments = "-c $($Pools.CryptoNight.Name)_CryptoNight.txt --noCPU --noNVIDIA"
