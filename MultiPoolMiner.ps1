...
while($true)
{
    ...
    $MinerCombos = Get-Combination $BestMiners ((Select Type,Index -Unique | Measure).Count, 7 | Measure -Minimum).Minimum | Where {$Combination = $_ | ForEach {$_.Combination | ForEach {$Miner = $_; $Miner.Type | ForEach {[PSCustomObject]@{Type=$_; Index=$Miner.Index}}}}; $Combination.Count -eq ($Combination | Select * -Unique).Count}
    $MinerCombos_Comparison = Get-Combination $BestMiners_Comparison ((Select Type,Index -Unique | Measure).Count, 7 | Measure -Minimum).Minimum | Where {$Combination = $_ | ForEach {$_.Combination | ForEach {$Miner = $_; $Miner.Type | ForEach {[PSCustomObject]@{Type=$_; Index=$Miner.Index}}}}; $Combination.Count -eq ($Combination | Select * -Unique).Count}
