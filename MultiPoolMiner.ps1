...
while ($true) {
    ...
    $Miners | Where-Object {$_.Profit -ge 1E-5 -or $_.Profit -eq $null} | Sort-Object -Descending Type, Profit_Bias | Format-Table -GroupBy Type (
        ...
        @{Label = "$($Currency[0])/Day"; Expression = {$_.Profits.PSObject.Properties.Value | ForEach-Object {if ($_ -ne $null) {$($_ * $Rates.$($Currency[0])) | ConvertTo-ReadableNumbers}else {"Benchmarking"}}}; Align = 'right'}, 
        @{Label = "Accuracy"; Expression = {$_.Pools.PSObject.Properties.Value.MarginOfError | ForEach-Object {(1 - $_).ToString("P0")}}; Align = 'right'}, 
        @{Label = "$($Currency[0])/GH/Day"; Expression = {$_.Pools.PSObject.Properties.Value.Price | ForEach-Object {($_ * $Rates.$($Currency[0]) * 1000000000) | ConvertTo-ReadableNumbers}}; Align = 'right'}, 
