...
class XmRigCfgFile : Miner {
    ...
    hidden StartMining() {
        ...
        if (-not $Parameters.ConfigFile.Content.threads) {
            ...
            if (-not $ThreadsConfig.Count) {
                ...
                if (($ThreadsConfig.index).Count -le 1) {
                    ...
                    if ($this.Process | Get-Job -ErrorAction SilentlyContinue) {
                        for ($WaitForThreadsConfig = 0; $WaitForThreadsConfig -le 10; $WaitForThreadsConfig++) {
            ...
            $Parameters.ConfigFile.Content | Add-Member threads ([Array]($ThreadsConfig | Where-Object {$Parameters.Devices -contains $_.index})) -Force
